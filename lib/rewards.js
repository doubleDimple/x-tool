  ...
  const walk = (node, path = "") => {
    ...
    for (const [key, value] of Object.entries(node)) {
      ...
      if (typeof value === "number") {
        if (/impression/i.test(k) && looksLikeImpressionTotal(value) && found.impressions90d == null) {
      ...
      if (typeof value === "string" && /impression/i.test(k)) {
...
export async function scrapeOpenXTabs() {
  ...
  const preferred = [
    ...tabs.filter((tab) => /monetization|premium|creator|analytic|studio/i.test(tab.url || "")),
    ...tabs,
  ];
  for (const tab of preferred) {
    ...
    try {
      ...
      if (data && (data.impressions90d != null || data.premium != null || data.verifiedFollowers != null)) {
