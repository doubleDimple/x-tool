...
async function runFriendship(users, { onProgress, signal, auth: passedAuth }, act) {
  const list = asUserList(users).filter((user) => user && String(user.id || "").trim());
  if (!list.length) {
    throw new XError("HTTP", "No users to process");
  }

  const waited = Date.now();
  while (friendshipInFlight) {
    if (signal?.aborted) throw new DOMException("Aborted", "AbortError");
    if (Date.now() - waited > 8000) {
  ...
  try {
    const auth = passedAuth?.csrf ? passedAuth : await getAuth();
    if (!auth.csrf || !auth.authToken) {
      throw new XError("NOT_AUTHENTICATED", "Please log in to x.com first");
    }
    ...
    for (let i = 0; i < list.length; i += 1) {
      ...
      const emit = (extra = {}) => {
        onProgress?.({
          ...
          left: list.length - ok.length - fail.length,
          ...extra,
        });
      };
      emit({ waiting: "look" });
      ...
      let done = false;
      try {
        await act(user, auth.csrf, signal);
        ok.push(user);
        done = true;
      } catch (error) {
        if (error?.name === "AbortError") throw error;
        if (error?.code === "RATE_LIMITED") {
          await waitForRateLimit(error, emit, signal);
          try {
            await act(user, auth.csrf, signal);
            ok.push(user);
            done = true;
          ...
          await pauseLikeHuman("retry", signal, emit);
          try {
            await act(user, auth.csrf, signal);
            ok.push(user);
            done = true;
