...
let unfollowInFlight = false;

export async function runUnfollow(users, { onProgress, signal }) {
  if (unfollowInFlight) {
    throw new XError("BUSY", "An unfollow batch is already running");
  }
  unfollowInFlight = true;
  ...
  try {
    ...
    const ok = [];
    const fail = [];
    const restEvery = randInt(7, 11);

    for (let i = 0; i < users.length; i += 1) {
      if (signal?.aborted) throw new DOMException("Aborted", "AbortError");
      const user = users[i];
      onProgress?.({ index: i, total: users.length, user, waiting: "look" });
      await pauseLikeHuman("look", signal, onProgress);

      let done = false;
      try {
        await unfollowUser(user.id, auth.csrf, signal);
        ok.push(user);
        done = true;
      } catch (error) {
        if (error?.name === "AbortError") throw error;
      ...
      if (done) onProgress?.({ index: i, total: users.length, user, done: true });
      if (i >= users.length - 1) continue;

      await pauseLikeHuman("between", signal, onProgress);
      if ((ok.length + fail.length) % restEvery === 0) {
        await pauseLikeHuman("rest", signal, onProgress);
      }
    }

    return { ok, fail };
  } finally {
    unfollowInFlight = false;
  }
}
