...
export function setCapturedOverride(captured) {
  capturedOverride = captured && typeof captured === "object" ? captured : null;
}

export async function getCaptured() {
  if (capturedOverride) return capturedOverride;
  const { captured } = await storageGet("captured");
  return captured || {};
}

async function getQueryIds() {
  const captured = await getCaptured();
  const { queryIds } = await storageGet("queryIds");
  return {
    ...DEFAULT_QUERY_IDS,
    ...(queryIds || {}),
    ...(captured.queryIds || {}),
...
function isPageFetchResult(viaPage) {
  return Boolean(viaPage && typeof viaPage.status === "number" && "text" in viaPage);
}
...
function friendshipBody(userId, screenName) {
  const params = new URLSearchParams({ ...FRIENDSHIP_FIELDS, user_id: String(userId) });
  if (screenName) params.set("screen_name", String(screenName));
  return params.toString();
}

function assertFriendshipResult(data, action) {
  if (!data || typeof data !== "object") {
    throw new XError("HTTP", "Empty friendship response");
  }
  if (Array.isArray(data.errors) && data.errors.length) {
    const msg = data.errors[0]?.message || JSON.stringify(data.errors[0]);
    throw new XError("HTTP", msg);
  }
  const following = data.following ?? data.relationship?.source?.following;
  if (action === "destroy" && following === true) {
    throw new XError("HTTP", "Still following");
  }
  if (action === "create" && following === false) {
    throw new XError("HTTP", "Not following");
  }
  if (following == null && !data.id_str && !data.screen_name && !data.id) {
    throw new XError("HTTP", "Friendship response missing user");
  }
}
...
async function friendshipPost(userId, csrf, signal, action, screenName) {
  if (!userId || !/^\d+$/.test(String(userId))) {
    throw new XError("HTTP", `Bad user id: ${userId || "(empty)"}`);
  }
  const captured = await getCaptured();
  const body = friendshipBody(userId, screenName);
  ...
  if (isPageFetchResult(viaPage) && viaPage.ok) {
    const data = viaPage.text ? parseJsonBody(viaPage.text, viaPage.status) : {};
    assertFriendshipResult(data, action);
    return data;
  }
  ...
  const text = await response.text();
  if (response.ok) {
    const data = text ? parseJsonBody(text, response.status) : {};
    assertFriendshipResult(data, action);
    return data;
  }
  if (isPageFetchResult(viaPage) && viaPage.status >= 400) {
...
function friendshipTarget(user) {
  if (user && typeof user === "object") {
    return { id: user.id, screenName: user.screenName };
  }
  return { id: user, screenName: "" };
}

export async function unfollowUser(user, csrf, signal) {
  const { id, screenName } = friendshipTarget(user);
  return friendshipPost(id, csrf, signal, "destroy", screenName);
}

export async function followUser(user, csrf, signal) {
  const { id, screenName } = friendshipTarget(user);
  return friendshipPost(id, csrf, signal, "create", screenName);
}
