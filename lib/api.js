...
async function fetchViaPage(url, headerMap) {
...
async function xfetch(url, csrf, { signal, extraHeaders } = {}) {
  const captured = await getCaptured();
  ...
  if (response.status === 403 || response.status === 404) {
    const viaPage = await fetchViaPage(url, headerMap);
    if (viaPage?.ok && viaPage.text) return parseJsonBody(viaPage.text, viaPage.status);
    if (viaPage?.status) {
...
export function normalizeUser(raw, source = "graphql") {
  ...
  if (source === "rest") {
    return {
      ...
      name: raw.name,
      avatar: (raw.profile_image_url_https || "").replace("_normal", "_bigger"),
      bio: raw.description || "",
  ...
  const user = raw.result && raw.result.legacy ? raw.result : raw;
  const legacy = user.legacy || {};
  if (!user.rest_id && !legacy.screen_name) return null;
  if (user.__typename === "UserUnavailable") return null;
  return {
    id: String(user.rest_id || legacy.id_str || ""),
    ...
    followingThem: Boolean(legacy.following),
  };
}
...
function walkUsersAndCursor(node, acc) {
  ...
  const result = node.user_results?.result || (node.__typename === "User" ? node : null);
  if (result && (result.legacy || result.rest_id) && result.__typename !== "UserUnavailable") {
    const user = normalizeUser(result, "graphql");
    if (user?.id && !acc.seen.has(user.id)) {
...
export function parseGraphQlList(data) {
  ...
  const timeline =
    ...
    root?.timeline ||
    data;
  const acc = { users: [], nextCursor: null, seen: new Set() };
  ...
  return { users: acc.users, nextCursor: acc.nextCursor };
}
...
export async function fetchListPage({ kind, userId, cursor, csrf, prefer, signal }) {
  ...
  for (const mode of order) {
    try {
      if (mode === "graphql") {
        return { ...(await graphqlList(operation, userId, cursor, csrf, signal)), mode: "graphql" };
      }
...
export async function verifyCredentials(csrf, signal) {
  ...
  return {
    ...
    name: data.name,
    avatar: (data.profile_image_url_https || "").replace("_normal", "_bigger"),
...
export async function getUserByScreenName(screenName, csrf, signal) {
  ...
  return {
    ...
    avatar: (user.legacy?.profile_image_url_https || "").replace("_normal", "_bigger"),
...
export async function discoverQueryIds(csrf, signal) {
  ...
  const found = {};
  const re =
    /(?:queryId|queryID)["':]+([A-Za-z0-9_-]{10,})["']?[^.]{0,80}operationName["':]+(Following|Followers|UserByScreenName)\b|\boperationName["':]+(Following|Followers|UserByScreenName)[^.]{0,80}(?:queryId|queryID)["':]+([A-Za-z0-9_-]{10,})/g;
  ...
  for (const url of unique) {
    ...
    for (const match of text.matchAll(re)) {
      ...
      if (id && name) found[name] = id;
    }
    if (found.Following && found.Followers && found.UserByScreenName) break;
  }
